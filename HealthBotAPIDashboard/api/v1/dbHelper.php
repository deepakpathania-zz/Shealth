...
class dbHelper {
    ...
    function select2($table, $columns, $where, $order){
        ...
        function selectjoin($table1,$table2, $col, $where){
        try{
            $a = array();
            $w = "";
            foreach ($where as $key => $value) {
            ...
            $stmt = $this->db->prepare("select * from ".$table1." inner join ".$table2 ." on ".$table1.".".$col."=".$table2.".".$col. " having  1=1  ". $w);
            $stmt->execute($a);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if(count($rows)<=0){
                $response["status"] = "warning";
        ...
        function selectjoin2($table1,$table2, $col1,$col2, $where){
        try{
            $a = array();
            $w = "";
            foreach ($where as $key => $value) {
